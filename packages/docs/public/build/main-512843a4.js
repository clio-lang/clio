...
function dynamicImport(path) {
  switch (path) {
    ...
    case "./routes/v0.9.0/index.svelte":
      return import('./index-a222f77a.js');
  }
}
...
function create_each_block(ctx) {
	...
	const block = {
		c: function create() {
			...
			t0 = text(t0_value);
			t1 = space();
			add_location(h4, file, 72, 10, 1949);
...
function create_fragment(ctx) {
	...
	const block = {
		c: function create() {
			...
			add_location(main, file, 56, 2, 1498);
			attr_dev(h3, "class", "svelte-1rp6xr7");
			add_location(h3, file, 65, 6, 1751);
			attr_dev(div0, "class", "sticky svelte-1rp6xr7");
			add_location(div0, file, 64, 4, 1724);
			attr_dev(div1, "class", "headnav svelte-1rp6xr7");
			add_location(div1, file, 63, 2, 1698);
			attr_dev(div2, "class", "page svelte-1rp6xr7");
			add_location(div2, file, 54, 0, 1432);
		},
		l: function claim(nodes) {
			throw new Error("options.hydrate only works if the component was compiled with the `hydratable: true` option");
		},
...
function instance($$self, $$props, $$invalidate) {
	...
	const setMetaVariant = variant => {
		if (variant) dynamicImport(`./meta/${variant}.json`).then(data => data.default).then(data => console.log(data)).then(meta => metaVariant = meta);
	};

	const slug = title => slugify(title).toLowerCase();

	const getComponent = () => async () => {
		console.log({ metaVariant });
		return await import('./index-a222f77a.js');
	};
...
//# sourceMappingURL=main-512843a4.js.map
