...
test("Import config file", () => {
  const config = packageConfig.getPackageConfig(path.join(__dirname, "cliopkg.test.toml"));
...
test("Write config file", () => {
  ...
  const file = fs.readFileSync(path.join(filePath, packageConfig.configFileName));
